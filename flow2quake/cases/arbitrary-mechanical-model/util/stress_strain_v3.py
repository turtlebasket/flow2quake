...
def create_reservoir_object(RES_file,block_size=500) :
    print ('Creating Reservoir object...', end = " ")
    rsrvr = Reservoir(RES_file,block_size)

    BLOCKS            = rsrvr.BLOCKS
    POINTS            = BLOCKS[['X','Y','Z']]
    POINTS['X']       = BLOCKS['X'] + BLOCKS['wX']/2
    POINTS['Y']       = BLOCKS['Y'] + BLOCKS['wY']/2
    POINTS['Z']       = rsrvr.BLOCKS['Z']-10

    rsrvr.StrainVolume = StrainVolume()
    rsrvr.StrainVolume.LoadBlocks(BLOCKS)
    rsrvr.StrainVolume.LoadPoints(POINTS)

    print ('[DONE]')

    return rsrvr

def compute_green_functions(folder,Reservoir_object) :
    """Computes and saves the matrix we use to compute the stress tensor"""
    rsrvr = Reservoir_object
    
    print ('Computing GreenFunction')
    rsrvr.StrainVolume.compute(GreenFunction=True, dv='cpu')
    torch.save (rsrvr.StrainVolume.Disp, folder+'G_matrix_disp.pt')
    torch.save (rsrvr.StrainVolume.σ, folder+'G_matrix_sigma.pt')
    print('Greens functions saved to {} folder'.format(folder))


def compute_coulomb_stress(Reservoir_object, RES, MESH, pressures_list, depth_above_res=10,activate_cuda=False) :
    """Compute the coulomb stress using the matrix computed with compute_green_functions()"""
    
    rsrvr = Reservoir_object

    print ('Loading G matrix...')
    rsrvr.StrainVolume.σ = torch.load('reservoir_data/G_matrix_sigma.pt')
    rsrvr.StrainVolume.Disp = torch.load('reservoir_data/G_matrix_disp.pt')
    ...
    values = np.empty((RES['X'].shape[0], RES['X'].shape[1], len(pressures_list)))
    ...
    values = rsrvr.DepthSlice_FullTEvolution_V2('./', pres_res_mesh, depth = RES['RES']['Depth'].flatten() - depth_above_res, activate_cuda=activate_cuda)
...
class StrainVolume:
    ...
    def _BoundaryPerturbation(self,BoundaryTolerance,tol=1e-3):
        ...
        #xmin corresponds to a 1D table filled with the xmin values for each block
        #same for xmax, ymin...
        ...
        #tqdm : progress bar
        #iteration over every reservoir block
        for jj in tqdm(range(self.vertices.shape[0]), desc='Observation Points Boundary Check'):
            ...
            #--> select points belonging to the face xmin of the block
            ...
            #Perturbation of x for XminIndex and XmaxIndex
            #Same for all y and z
            #Each of the 6 block faces is perturbed
            ...
            #InVolIndex : all the obspoints within the volume of block jj
            ...
            try:
                ...
                InVol_value[InVolIndex] = self.dP[jj] 
            except:
                pass

        #InVol : contains True for all observation points within a reservoir block
        #InVol_value the pressure for each InVol point
    ...
    def LoadBlocks(self,BLOCKS,PoissonRatio=0.25,BiotCoef=1.0,ShearModulus=6e9):
        ...
        #for each block, self.vertices contains the 3 coordinates of each of the 8 summits
        ...
        self.ν = PoissonRatio # Poisson's ratio
        self.α = BiotCoef     # Biot's coefficient
        self.G = ShearModulus # Shear Modulus #added after problem with coulomb stress by VJC
        self._elastic_constant() #created the shear modulus, young's modulus and bulk modulus from nu and alpha
    ...
    def compute(self,verbrose=True,dv='cpu',GreenFunction=False):
        ...
        Then goes back to the previous coordinates 
        """
    ...
    def PointValues(self,device='cpu',GreenFunction=False):
        ...
        self.obspoints_inBlock = self.obspoints_inBlock.to(dv) #added by VJC
...
class Reservoir:
    def __init__(self,ReservoirFile,block_size=500):


        # Loading information into block info
        self.RES                  = np.load(ReservoirFile,allow_pickle=True).item()
        ...
        from matplotlib.patches import Circle, Wedge, Polygon
        self.RES['Motif'] = MotifReservoir #MotifReservoir contains ones for all points inside reservoir and zeros elsewhere
        del MotifReservoir

        #self.BLOCKS is a dataframe containing position, dimensions, pressure and compressibility for each block
        ...
        self.BLOCKS['wX']         = self.BLOCKS['wY'] = block_size
        ...
        self.BLOCKS['Cm']         =  ((self.RES['Cm']['RAD'] + self.RES['Cm']['OL'] + self.RES['Cm']['InSARGPS'])/3).flatten()#np.array([[3e-11 for j in range(70)] for i in range(64)]).flatten()
    ...
    def SingleBlock(self,PATH,index=0,sep=5,extr=100):
        BLOCK = self.BLOCKS.iloc[index] #Block contains X, dX, Y, dY, Z, dZ, dP/dStrain, Cm for the chosen block index

        #Creation of a meshgrid on X and Z of the blocksizewith an additionnal margin on the sides
        X,Z = np.meshgrid(np.arange(BLOCK['X']-extr,BLOCK['X']+BLOCK['wX']+extr,sep),np.arange(BLOCK['Z']-extr,BLOCK['Z']+BLOCK['wZ']+extr,sep))
        #POINTS_np contains the meshgrid points coordinates
        POINTS_np = np.zeros((len(X.flatten()),3))
        POINTS_np[:,0] = X.flatten()
        POINTS_np[:,1] = BLOCK['Y']+BLOCK['wY']/2 #Y is fixedwith its value in the middle of the block : crosssection over Y + wY/2
        POINTS_np[:,2] = Z.flatten()
        POINTS = pd.DataFrame(POINTS_np,columns=['X','Y','Z'])

        #Call StrainVolume for the calculations
        self.StrainVolume = StrainVolume() #Creation of a StrainVolume object
        self.StrainVolume.LoadBlocks(BLOCK) #Creation of self.vertices (coordinates of the blocks' summits),
        # of self.dP, self.BlockThick and self.Cm , as well as the elastic constants
        self.StrainVolume.LoadPoints(POINTS)
        self.StrainVolume.compute() #Calculates the deformations and stresses for each observation point

        # # -- Putting Output into Numpy array for plotting
        Disp            = self.StrainVolume.Disp.numpy()
        Stress          = np.zeros((self.StrainVolume.σ.shape[0],3,3)) #3, 3 due to the tensor size, only 6 components are full due to the tensor's symmetry
    ...
    def DepthSlice(self,PATH,depth=0.0,mu=0.3,Strike=270,Dip=85,smoothing=6.0,GreenFunction=False,verbrose=True):
        BLOCKS            = self.BLOCKS
        #Observation points : top view of a horizontal crosssection at a given depth.
        #X and Y coordinates of the observation points correspond to the centers of the reservoir blocks
        POINTS            = BLOCKS[['X','Y','Z']]
        POINTS['X']       = BLOCKS['X'] + BLOCKS['wX']/2
        POINTS['Y']       = BLOCKS['Y'] + BLOCKS['wY']/2
        POINTS['Z']       = depth
        ...
        if verbrose:
          ...
          #Normal, shear and Coulomb stresses
          #Normal stress : n.σ.n
          Stress_faultnormal  = np.dot(np.dot(Stress,Fault_normal),Fault_normal)
          #Shear stress : σ.n - (n.σ.n)n (total stress on normal plane minus normal part)
          Stress_faultshear   = np.sqrt(np.sum(np.dot(Stress,Fault_normal)**2,axis=-1) - Stress_faultnormal**2)
          #obspoints_inBlock_value contiains pressure values in the reservoir
          #Coulomb stress : shear stress - friction coeeficient * (normal stress - pore pressure)
          Stress_faultcoulomb = (Stress_faultshear - mu*(Stress_faultnormal-self.StrainVolume.obspoints_inBlock_value.numpy()))/1e6
          ...
          #Max shear : difference between maximum and minimum eigen values /2
          Stress_maxshear  = 0.5*(np.max(np.linalg.eigvals(np.nan_to_num(Stress)),axis=-1) - np.min(np.linalg.eigvals(np.nan_to_num(Stress)),axis=-1))
          #Max normal : Normal stress as (σmax + σmin)/2 , used to calculate coulomb stress at point where maximum shear stress is reached
          Stress_maxnormal = 0.5*(np.max(np.linalg.eigvals(np.nan_to_num(Stress)),axis=-1) + np.min(np.linalg.eigvals(np.nan_to_num(Stress)),axis=-1))
          #Calculate coulomb stress at point where maximum shear stress is reached
          Stress_maxcoulomb = (Stress_maxshear - mu*(Stress_maxnormal-self.StrainVolume.obspoints_inBlock_value.numpy()))/1e6
          Stress_maxcoulomb[Stress_maxcoulomb==np.inf]=0
          ...
          #SAME WITH GAUSSIAN KERNEL SMOOTHING
          kernel = Gaussian2DKernel(smoothing)
    ...
    def DepthSlice_FullTEvolution_V2(self,PATH, PRES, activate_cuda=False, depth=0.0,mu=0.3,Strike=270,Dip=85,smoothing=6.0,verbrose=False):
        ...
        #MAX COULOMB STRESS : VALENTIN'S MODIFICATION
